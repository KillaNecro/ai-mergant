...
const StatusBadge = ({ p }) => {
  if (p.is_edited) return <span className="inline-flex items-center rounded-full bg-blue-50 text-blue-700 px-2 py-0.5 text-xs font-medium border border-blue-100">Düzenlendi</span>;
  if (!p.description || p.price == null) return <span className="inline-flex items-center rounded-full bg-amber-50 text-amber-700 px-2 py-0.5 text-xs font-medium border border-amber-100">Eksik</span>;
  return <span className="inline-flex items-center rounded-full bg-emerald-50 text-emerald-700 px-2 py-0.5 text-xs font-medium border border-emerald-100">Tamam</span>;
};
    ...
    <div className="space-y-6" data-testid="products-page">
      ...
      <div className="bg-white border border-slate-200 rounded-md shadow-sm overflow-hidden">
        <table className="w-full text-sm text-left border-collapse">
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={10} className="px-4 py-10 text-center text-slate-500">Yükleniyor...</td></tr>
            )}
            {!loading && error && (
              <tr><td colSpan={10} className="px-4 py-10 text-center">
                ...
                <div className="mt-2"><button onClick={load} className="text-sm font-medium text-blue-700 hover:text-blue-800">Tekrar dene</button></div>
              </td></tr>
            )}
            ...
            {!loading && !error && data.items.map((p) => (
              <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50/60 text-slate-700">
                <td className="px-4 py-3">
                  <input type="checkbox" data-testid={`row-select-${p.sku}`}
                ...
                <td className="px-4 py-3">
                  ...
                  {p.improved_name && <div className="text-xs text-slate-400 line-clamp-1">Orijinal: {p.name}</div>}
                </td>
                ...
                <td className="px-4 py-3 text-xs text-slate-500">{new Date(p.updated_at).toLocaleString("tr-TR")}</td>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 bg-slate-50/50 text-sm text-slate-600">
          <div title="Bu sayfadaki ürünleri seç">
            Sayfa {page} / {totalPages} · <span className="text-slate-400">Tümü Seç yalnızca bu sayfayı seçer</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      {editing && (
        <ProductEditor productId={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />
      )}
    </div>
  );
};
