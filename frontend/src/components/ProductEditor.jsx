...
import { X, Wand2, RotateCcw, Save, Sparkles } from "lucide-react";
...
const ProductEditor = ({ productId, onClose, onSaved }) => {
  ...
  useEffect(() => {
    api.get(`/products/${productId}`).then((r) => setP(r.data));
  }, [productId]);
      ...
      >
        <div className="h-14 flex items-center justify-between px-6 border-b border-slate-200">
          <div>
            ...
            <div className="font-semibold text-[#0A1128] tracking-tight">{p.improved_name || p.name}</div>
          </div>
        ...
        <div className="p-6 space-y-6">
          <div className="flex flex-wrap gap-2">
            ...
            <button data-testid="revert-btn" onClick={revert}
              className="bg-white border border-slate-200 hover:bg-slate-50 text-[#0A1128] rounded-md px-3.5 py-2 text-sm font-medium inline-flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 min-h-[42px]">
                {p.name}
              </div>
            </div>
            <div className="space-y-2">
              ...
              <input
                data-testid="improved-name-input"
                value={p.improved_name || ""}
                onChange={(e) => set("improved_name", e.target.value)}
            ...
            <div className="space-y-2">
              ...
              <textarea
                data-testid="improved-desc-input"
                value={p.improved_description || ""}
                onChange={(e) => set("improved_description", e.target.value)}
                rows={8}
                placeholder="Henüz iyileştirilmedi"
                className="w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
